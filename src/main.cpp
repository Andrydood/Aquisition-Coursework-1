...
int main(int argc, char *argv[]) {
    ...
    std::string meshPath = {"../3rdparty/libigl/tutorial/shared/bunny.off", "../3rdparty/libigl/tutorial/shared/bunny.off"} ;
    if (argc > 1) {

        for(int i =0; i<argc; i++){
            std::string meshPath[i] = std::string(argv[i+1]);
        }
        if (meshPath[0].find(".off") == std::string::npos) {
    ...
    // Read mesh from meshPath
    {
        ...
        Eigen::MatrixXi F;
        // Read mesh
        igl::readOFF(meshPath[0], V, F);
        // Check, if any vertices read
        if (V.rows() <= 0) {
            std::cerr << "Could not read mesh at " << meshPath[0]
                      << "...exiting...\n";
            return EXIT_FAILURE;
        } //...if vertices read
